...
mod tests {
    ...
    fn test_qe_start_to_end() {
        ...
        if original {
            ...
            println!("Time elapsed in qe_enc_matrix_samedim is: {:?}", end);
            ...
            y_xmu_1_1.push(Integer::from(1));
            // println!("y_x_1_1 = {:?}", y_xmu_1_1);
            
            // println!("enc_mat_x size = {} x {}", enc_mat_x.rows, enc_mat_x.cols);
            // println!("enc_mat_y size = {} x {}", enc_mat_y.rows, enc_mat_y.cols);
            // println!("enc_mat_h size = {} x {}", enc_mat_h.rows, enc_mat_h.cols);
            // println!("x_mu_1 size = {}", x_mu_1.len());
            // println!("y1 size = {}", y1.len());
            // println!("y_x_1_1 size = {}", y_xmu_1_1.len());

            ctxt_x = enc_mat_x.mul_vec(&x_mu_1);
        ...
        let start = SystemTime::now();
        let out = qe_dec((&sk_f, &sk_red), (&ctxt_x, &ctxt_y, &ctxt_h), &grp);
        let end = start.elapsed();
